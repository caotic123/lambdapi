...
let _ =
  ...
  let eq_noexn : term -> term -> bool =
   fun t u ->
    let p = new_problem () in
    p := { !p with to_solve = [ ([], t, u) ] };
...
(** [rec_open record ss p] opens [p] and all its dependencies marked as open. It
    assumes that [p] and all its dependencies are already loaded. Records open
    modules if [record]. On success, an updated signature state is returned. *)
...
(** [rec_require compile ss p] handles the command [require p] with [ss] as
    signature state and [compile] as compilation function (passed as argument to
    avoid cyclic dependencies). On success, an updated signature state is
    returned. *)
...
let handle_require_as : compiler -> sig_state -> p_path -> p_ident -> sig_state
    =
...
let handle_require compile bo ss { elt = p; _ } =
  let ss = rec_require compile ss p in
  match bo with Some prv -> rec_open (not prv) ss p | None -> ss

(** [handle_modifiers ms] verifies that the modifiers in [ms] are compatible. If
    so, they are returned as a tuple. Otherwise, it fails. *)
 ...
 fun ms ->
  let rec get_modifiers ((props, expos, strats) as acc) = function
    ...
    | ({ elt = P_prop _; _ } as p) :: ms ->
      get_modifiers (p :: props, expos, strats) ms
    | ({ elt = P_expo _; _ } as e) :: ms ->
      get_modifiers (props, e :: expos, strats) ms
  ...
  let prop =
    ...
    | [ { elt = P_prop Commu; _ }; { elt = P_prop (Assoc b); _ } ] ->
      AC b
    | _ :: { pos; _ } :: _ -> fatal pos "Incompatible or duplicated properties."
    | [ { elt = P_prop (Assoc _); pos } ] ->
      fatal pos
        "Associativity alone is not allowed as you can use a rewriting rule \
         instead."
    | [ { elt = P_prop p; _ } ] -> p
    | [] -> Defin
    | _ -> assert false
...
(** [handle_inductive_symbol ss e p strat x xs a] handles the command
    [e p strat symbol x xs : a] with [ss] as the signature state. The command is
    at position [pos]. On success, an updated signature state and the new symbol
    are returned. *)
let handle_inductive_symbol :
 ...
 fun ss expo prop mstrat ({ elt = name; pos } as id) declpos xs typ ->
  ...
  let r =
    Sig_state.add_symbol ss expo prop mstrat false id declpos typ impl None
  in
  sig_state := fst r;
  r
...
(** Representation of a yet unchecked proof. The structure is initialized when
    ...
    is exited (i.e., when a terminator like “end” is used). Note that tactics do
    not work on this structure directly, although they act on the contents of
    its [pdata_state] field. *)
...
(** [get_proof_data compile ss cmd] tries to handle the command [cmd] with [ss]
...
let get_proof_data : compiler -> sig_state -> p_command -> cmd_output =
 fun compile ss ({ elt; pos } as cmd) ->
  ...
  | P_when (_head, (_subj, _typ), _conseq) ->
    let sr = scope_typed_contraints false ss (_head, (_subj, _typ), _conseq) in
    let sr = if Stdlib.(!sr_check) then Tool.Sr.check_constraint sr.cr_pos sr else sr in
    (* A constraint belongs to no symbol — store it in the signature's flat
       list of typed constraints. *)
    Sign.add_type_constrains ss.signature sr;
    (ss, None, None)
  | P_require (bo, ps) ->
  ...
  | P_rules rs ->
    ...
    let handle_rule r (srs, map) =
      ...
      let ((s, r) as sr) =
        if Stdlib.(!sr_check) then Tool.Sr.check_rule r.pos sr else sr
      in
    ...
    if !Console.verbose >= 2 then
      List.iter (Console.out 2 (Color.gre "rule %a") sym_rule) (List.rev srs);
  ...
  | P_notation (qid, n) ->
    ...
    | (Assoc false | AC false), Infix (Pratter.Left, _) ->
      fatal pos
        "notation incompatible with symbol property (e.g. infix right notation \
         on left associative symbol)"
    | _ -> ());
  ...
  | P_inductive (ms, params, p_ind_list) ->
    ...
    if mstrat <> Eager then
      fatal pos
        "Pattern matching strategy modifiers cannot be used on inductive types.";
    ...
    let add_ind_sym (ss, ind_sym_list) { elt = id, pt, _; _ } =
      let ss, ind_sym =
        ...
        handle_inductive_symbol ss expo Const Eager id pos params pt
      in
      (ss, ind_sym :: ind_sym_list)
    in
    let ss, ind_sym_list_rev = List.fold_left add_ind_sym (ss, []) p_ind_list in
    (* Set parameters as implicit in the type of constructors. *)
    let params =
      List.map (fun (idopts, typopt, _) -> (idopts, typopt, true)) params
    in
    (* Add constructors in the signature. *)
    let add_constructors (ss, cons_sym_list_list) { elt = _, _, p_cons_list; _ }
        =
      ...
      let ss, cons_sym_list_rev =
        List.fold_left add_cons_sym (ss, []) p_cons_list
      in
    ...
    let vs, env, ind_pred_map =
      Inductive.create_ind_pred_map pos cfg ind_nb_params ind_list a_str p_str
        x_str
    in
    ...
    let add_recursor (ss, rec_sym_list) ind_sym rec_typ =
      ...
      let ss, rec_sym =
        Console.out 2 (Color.gre "symbol %a : %a") uid rec_name term rec_typ;
        (* Recursors are declared after the types and constructors. *)
        let pos = after (pos_end pos) in
        let id = Pos.make pos rec_name in
        let r =
          Sig_state.add_symbol ss expo Defin Eager false id None rec_typ [] None
        in
        sig_state := fst r;
        r
      in
      ...
      | Some pt ->
        let pt =
          if p_sym_arg = [] then pt
            ...
            let pos = Pos.(cat (pos_end p_sym_nam.pos) pt.pos) in
      ...
      | Some a ->
        let a =
          if p_sym_arg = [] then a
            ...
            let pos = Pos.(cat (pos_end p_sym_nam.pos) a.pos) in
            Pos.make pos (P_Prod (p_sym_arg, a))
        in
        (Some (scope ~typ:true a), Syntax.get_impl_term a)
    in
    ...
    let pdata, qres =
      ...
      let t, a =
        ...
        | Some { elt = a; pos } -> (
          ...
          match Infer.check_sort_noexn p [] a with
          ...
          | Some (a, _) ->
            ...
            (t, a))
        | None -> (
          (* If no type is given, infer it from the definition. *)
          match t with
          | None -> assert false
          | Some { elt = t; pos } -> (
      ...
      let pdata_finalize ss ps =
        ...
        | P_proof_admitted ->
          ...
          fst
            (Sig_state.add_symbol ss expo prop mstrat opaq p_sym_nam declpos a
               impl d)
        | P_proof_end ->
      ...
      let pdata_state =
        ...
        let ps =
          ...
          else { proof_name = p_sym_nam; proof_term = None; proof_goals }
        in
        Tactic.tac_solve pos ps
      in
      if p_sym_prf = None && not (finished pdata_state) then
        wrn pos "Some metavariables could not be solved: a proof must be given";
      ( {
          pdata_sym_pos = p_sym_nam.pos;
          pdata_state;
          pdata_proof;
          pdata_finalize;
          pdata_end_pos = pe.pos;
          pdata_prv;
        },
        qres )
    in
...
(** [handle compile ss cmd] retrieves proof data from [cmd] (with
    ...
    {!module:Tactic} The function [compile] is used to compile required modules
    recursively. *)
let handle : compiler -> Sig_state.t -> Syntax.p_command -> Sig_state.t =
 fun compile ss cmd ->
  ...
  | Some d ->
    let ps, _ =
      fold_proof
        (Tactic.handle ss d.pdata_sym_pos d.pdata_prv)
